...
from comde.comde_modules.environment_encoder.architectures.stochastic_lstm import StochasticLSTM


class BCZNonst(BaseLowPolicy):
	"""
		BCZ + Nonstationary: Use SPIRL type embedding
	"""
	...
	def __init__(self, seed: int, cfg: Dict, init_build_model: bool = True):
		super(BCZNonst, self).__init__(seed=seed, cfg=cfg, init_build_model=init_build_model)
	...
	def _build_encoder(self):
		encoder_cfg = self.cfg["encoder_cfg"].copy()
		lr = encoder_cfg.pop("lr")
		encoder = LSTM(**encoder_cfg)
		encoder = StochasticLSTM(encoder)
		b, l = 4, 3
		history_obs = np.zeros((b, l, self.observation_dim,))
		history_act = np.zeros((b, l, self.action_dim))
		encoder_input = np.concatenate((history_obs, history_act), axis=-1)
		n_iter = np.ones((b,))

		self.rng, rngs = get_basic_rngs(self.rng)
		carry_key, _ = jax.random.split(self.rng)
		rngs.update({"init_carry": carry_key})
		tx = optax.adam(learning_rate=lr)
		self.__encoder = Model.create(model=encoder, inputs=[rngs, encoder_input, n_iter], tx=tx)
	...
	def _excluded_save_params(self) -> List:
		return BCZNonst.PARAM_COMPONENTS

	def _get_save_params(self) -> Dict[str, Params]:
		params_dict = {}
		for component_str in BCZNonst.PARAM_COMPONENTS:
