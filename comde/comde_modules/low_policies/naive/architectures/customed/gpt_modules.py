...
class FlaxGPT2ModuleWoTimePosEmb(nn.Module):
	"""
	GPT2Module \ {Positional embedding, Timestep embedding}
	"""
	config: GPT2Config
	dtype: jnp.dtype = jnp.float32
...
class FlaxGPT2ModelWoTimePosEmb(FlaxGPT2PreTrainedModel):
...
class FlaxGPT2LMHeadModule(nn.Module):
	...
	def setup(self):
		self.transformer = FlaxGPT2ModuleWoTimePosEmb(self.config, dtype=self.dtype)
